...
<div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-4">
    <h3 class="mb-0">Kas Masuk</h3>
    {{-- tombol tambah pakai modal biar tampilan lebih rapi --}}
    <button type="button" class="btn btn-sm btn-gold-outline px-4 py-2 fw-semibold" data-bs-toggle="modal" data-bs-target="#tambahKasMasukModal">
...
@foreach($data as $item)
    <div class="modal fade" id="edit{{ $item->id }}" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable modal-fullscreen-md-down">
            <form method="POST" action="/kas-masuk/{{ $item->id }}" class="w-100">
                @csrf
                @method('PUT')
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <input type="date" name="tanggal"
                           ...
                           <select name="sumber" class="form-select mb-2" required>
                            <option value="cash" {{ strtolower((string) $item->sumber) === 'cash' ? 'selected' : '' }}>Cash</option>
                            <option value="qris" {{ strtolower((string) $item->sumber) === 'qris' ? 'selected' : '' }}>QRIS</option>
                           </select>
...
<div class="modal fade" id="tambahKasMasukModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable modal-fullscreen-md-down">
        <form method="POST" action="/kas-masuk" class="w-100">
            @csrf
